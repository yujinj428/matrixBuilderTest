...
/**
 * @namespace      $h.barcodeMLKit
 * @memberof       $h
 * @author         Inswave Systems
 * @description    MLKit 를 이용한 barcode scanner
 */
if (typeof $h.barcodeMLKit !== "object") {
    $h.barcodeMLKit = {}
}

/**
 * @function      $h.barcodeMLKit.startBarcodeScanner
 * @description   barcode scanner start(mlkit)
 * @param         {Function} callback - 콜백함수
 * @param         {Object} options.useGallery - 기본 제공하는 gallery 이미지 버튼 활성화 여부
 * @param         {Object} options.useFlash - 기본 제공하는 flash 이미지 버튼 활성화 여부
 * @param         {Object} options.useClose - 기본 제공하는 close 이미지 버튼 활성화 여부
 * @example
 * var callback = function(result){ console.log(result); };
 * var option = {"useGallery":"true", "useFlash":"true", "useClose":"true"};
 * $h.barcodeMLKit.startBarcodeScanner(callback, option);
 */
$h.barcodeMLKit.startBarcodeScanner = function (callback, options) {
    try {
        wmatrix.plugin.barcodescannermlkit.startBarcodeScanner(callback, options);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.barcodeMLKit.closeBarcodeScanner
 * @description   barcode scanner close/stop(mlkit)
 * @param         {Function} callback - 콜백함수
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.barcodeMLKit.closeBarcodeScanner(callback);
 */
$h.barcodeMLKit.closeBarcodeScanner = function (callback) {
    try {
        wmatrix.plugin.barcodescannermlkit.closeBarcodeScanner(callback);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.barcodeMLKit.galleryBarcodeScanner
 * @description   gallery image use barcode scanner
 * @param         {Function} callback - 콜백함수
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.barcodeMLKit.galleryBarcodeScanner(callback);
 */
$h.barcodeMLKit.galleryBarcodeScanner = function (callback) {
    try {
        wmatrix.plugin.barcodescannermlkit.galleryBarcodeScanner(callback);
    } catch(e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.barcodeMLKit.flashlightBarcodeScanner
 * @description   startBarcodeScanner 실행 이후에만 동작
 * @param         {Function} callback - 콜백함수
 * @param         {Object} options.flashFlag - flash on/off(true/flase)
 * @example
 * var callback = function(result){ console.log(result); };
 * var option = {"flashFlag":"true"};
 * $h.barcodeMLKit.flashlightBarcodeScanner(callback, option);
 */
$h.barcodeMLKit.flashlightBarcodeScanner = function (callback, option) {
    try {
        wmatrix.plugin.barcodescannermlkit.flashlightBarcodeScanner(callback, option);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.barcodeMLKit.barcodeScannerStatus
 * @description   실행 여부 확인 값 (true : 사용중, false : 종료)
 * @param         {Function} callback - 콜백함수
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.barcodeMLKit.barcodeScannerStatus(callback);
 */
$h.barcodeMLKit.barcodeScannerStatus = function (callback) {
    try {
        wmatrix.plugin.barcodescannermlkit.barcodeScannerStatus(callback);
    } catch (e) {
        console.log(e);
    }
}
 ...
 * console.log("deviceType : "+deviceInfo.type);
 */
...
/**
 * @function      $h.device.getBatteryLevel
 * @description   device battery level
 * @param         {Function} callback - 콜백함수
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.device.getBatteryLevel(callback);
 */
$h.device.getBatteryLevel = function (callback) {
    try {
        device.getBatteryLevel(callback);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.device.isCharging
 * @description   현재 배터리가 충전 상태인지 확인
 * @param         {Function} callback - 콜백함수
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.device.isCharging(callback);
 */
$h.device.isCharging = function (callback) {
    try {
        device.isCharging(callback);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.device.registerBattery
 * @description   배터리 상태 확인 receiver register
 * @param         {Function} callback - 콜백함수
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.device.registerBattery(callback);
 */
$h.device.registerBattery = function (callback) {
    try {
        device.registerBattery(callback);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function      $h.device.unregisterBattery
 * @description   배터리 상태 확인 receiver unregister
 * @param         {Function} callback - 콜백함수
 * @example
 * var callback = function(result){ console.log(result); };
 * $h.device.unregisterBattery(callback);
 */
$h.device.unregisterBattery = function (callback) {
    try {
        device.unregisterBattery(callback);
    } catch (e) {
        console.log(e);
    }
}
...
/**
 * @function       $h.network.getCurrentWifi
 * @description    현재 연결되어 있는 wifi 의 ssid(name) 반환
 * @example
 * var callback = function(result){ console.log((result)); };
 * $h.network.getCurrentWifi(callback);
 */
$h.network.getCurrentWifi = function (callback) {
    try {
        navigator.connection.getCurrentWifi(callback);
    } catch (e) {
        console.log(e);
    }
}
 ...
 * @memberof       $h
 * @author         Inswave Systems
 * @description    Gemini 룰 위한 기능
 */
if (typeof $h.gemini !== "object") {
    $h.gemini = {};
}

/**
* @function      $h.gemini.init
* @description   gemini model 객체를 생성 한다.
* @param         {Function} callback - 콜백함수
* @param         {String} callback.statusCode - 코드
* @param         {String} callback.msg - 메시지
* @param         {Object} callback.data - 데이터
...
* @param         {String} option.model          wmatrix.plugin.gemini.model 의 상수를 이용하거나, 신규 gemini 모델이 나오면 해당 모델명으로 설정한다.
* @param         {Float}  options.temperature   값이 높을수록 창의적 / 낮을 수록 사실적
* @param         {Int} options.topK             확률 분포도 기반 sampling / K개 의 단어를 기준으로 선택 / 높을 수록 다양성 / 낮을 수록 일관적. (Int)
* @param         {Float} options.topP           확률 분포도 기반 sampling / P의 지점을 넘는 확률 기반으로 선택 / 높을 수록 다양한 답변 / 낮을 수록 일관적. (float)
* @param         {Int} options.maxOutputTokens  최대 token 수.
* @example
* let callback = function(r) { console.log(r); }
* let option = {}
* option.apiKey = ""
* option.model = wmatrix.plugin.gemini.model.gemini_1_5_flash
* option.temperature = 0.15;
* option.topK = 40;
* option.topP = 1;
* option.maxOutputTokens = 4096;
* $h.gemini.init(callback, option);
*/
$h.gemini.init = function (callback, options) {
    try {
        wmatrix.plugin.gemini.init(callback, options)
    } catch(e) {
        console.log(e);
    }
}
...
/**
* @function      $h.gemini.generateContent
* @description   AI 가 요구 사항에 따른 콘텐츠를 생성한다. (글쓰기 등.)
* @param         {Function} callback - 콜백함수
* @param         {String} callback.statusCode - 코드
* @param         {String} callback.msg - 메시지
* @param         {Object} callback.data - 생성 된 content data.
* @example
* let callback = function(r) { console.log(r); }
* $h.gemini.generateContent(callback, "고양이에 관한 시를 써줘.");
*/
$h.gemini.generateContent = function (callback, options) {
    try {
        wmatrix.plugin.gemini.generateContent(callback, options)
    } catch(e) {
        console.log(e);
    }
}
...
/**
* @function      $h.gemini.imagePrompting
* @description   AI 로 전달된 이미지와 관련 된 질의, 분석 등을 수행한다.
* @param         {Function} callback - 콜백함수
* @param         {String} callback.statusCode - 코드
* @param         {String} callback.msg - 메시지
* @param         {Object} callback.data - 생성 된 content data.
* @param         {Object} options - object
* @param         {String} options.base64Image - base64 형태의 이미지.
* @param         {String} options.prompt - 이미지와 연관 된 prompt.
* @example
* let callback = function(r) { console.log(r); }
* let options = {};
* options.base64Image = "base64 string image";
* options.prompt = "이미지 속의 제품이 뭐야?"
* $h.gemini.generateContent(callback, option);
*/
$h.gemini.imagePrompting = function (callback, options) {
    try {
        wmatrix.plugin.gemini.imagePrompting(callback, options)
    } catch(e) {
        console.log(e);
    }
}

$h.gemini.responseStream = function (callback, options) {
    try {
        wmatrix.plugin.gemini.responseStream(callback, options)
    } catch(e) {
        console.log(e);
    }
}
...
/**
* @function      $h.gemini.initChat
* @description   채팅을 하기 위해 모델을 채팅모드로 초기화 시킨다.
* @param         {Function} callback - 콜백함수
* @param         {String} callback.statusCode - 코드
* @param         {String} callback.msg - 메시지
* @param         {Object} callback.data - 생성 된 content data.
* @param         {Object} options - object
* @param         {String} options.history - json array 안에 json object 형태.
* @example
* let option = {}
* option.history = [];
* option.history.push({
*    "role": "user",
*    "content": "Hello, I have 2 dogs in my house."
* }, {
*    "role": "model",
*    "content": "Great to meet you. What would you like to know?"
* })
* $h.gemini.initChat(callback, option);
*/
$h.gemini.initChat = function (callback, options) {
    try {
        wmatrix.plugin.gemini.initChat(callback, options)
    } catch(e) {
        console.log(e);
    }
}
...
/**
* @function      $h.gemini.chat
* @description   gemini 와 채팅을 한다.
* @param         {Function} callback - 콜백함수
* @param         {String} callback.statusCode - 코드
* @param         {String} callback.msg - 메시지
* @param         {Object} callback.data - 생성 된 content data.
* @example
* let chat = "kotlin 코드를 작성해야 하는데 뭐부터 해야할까?"
* $h.gemini.chat(callback, chat);
*/
$h.gemini.chat = function (callback, options) {
    try {
        wmatrix.plugin.gemini.chat(callback, options)
    } catch(e) {
        console.log(e);
    }
}
...
/**
* @function      $h.gemini.clearChatHistory
* @description   채팅 히스토리를 삭제한다.
* @param         {Function} callback - 콜백함수
* @param         {String} callback.statusCode - 코드
* @param         {String} callback.msg - 메시지
* @param         {Object} callback.data - 생성 된 content data.
* @example
* $h.gemini.clearChatHistory(callback);
*/
$h.gemini.clearChatHistory = function (callback, options) {
    try {
        wmatrix.plugin.gemini.clearChatHistory(callback, options)
    } catch(e) {
        console.log(e);
    }
}
 ...
 * memberof       $h
 * author         Inswave Systems
 * description    firebase push 를 위한 기능
 */
if (typeof $h.fcm !== "object") {
    $h.fcm = {};
}

/**
* @function      $h.fcm.getToken
...
* @example
* var callback = function(result){ console.log(result); };
* $h.fcm.getToken(callback)
*/
$h.fcm.getToken = function(callback) {
    try {
        wmatrix.plugin.fcm.getToken(callback);
    } catch (e) {
      console.log(e);
    }
}